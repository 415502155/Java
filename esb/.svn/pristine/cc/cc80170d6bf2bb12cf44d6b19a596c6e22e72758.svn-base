...
<form id="pagerForm" method="post" action="${ctx}/manage/range/setheadmaster">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="${ctx}/manage/range/setheadmaster" method="post">
	<div class="searchBar">
				...
				<li><a href="javascript">${dep.dep_name}</a>
					...
					<ul>
						<c:forEach var="teacher" items="${dep.teachers}" varStatus="status">
							<li><a href="${ctx}/manage/range/getHeadmasterOrgs?tech_id=${teacher.tech_id}" target="ajax" rel="jbsxBox">${teacher.tech_name}</a></li>
						</c:forEach>						
					</ul>
					</c:if>
				</li>
