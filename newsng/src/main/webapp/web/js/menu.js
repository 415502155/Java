        ...
        "nextNav":{
            ...
            "f4":{
                ...
                "nextNav":{
                   ...
                   /* "f42":{
                        "name":"创建位置",
                        "url":"baseSet/classroom/addAddress.html",
                        "describe":"添加编辑教学楼信息页面！"
                    },*/
                    "f43":{
                        "name":"教室管理",
...
for (var i in nav){
    ...
    if(obj !== null){
        ...
        if(obj.isShow==true){
            ...
            //注册子路由
            for(var m in obj.nextNav){
                router[m]="html/"+obj.nextNav[m].url;

                if(obj.nextNav[m].nextNav!==undefined){
                    for(var n in obj.nextNav[m].nextNav){
                        var third=obj.nextNav[m].nextNav[n];
...
function creatThirdMenu(id) {
    ...
    if(defaultNav!==undefined){
        $("#firstlMenu").text(defaultNav.name);
    }else{
...
function countup(id,countSum){
    ...
    var flash = new CountUp(id, 0, countSum, 0, 1.8, options);
    flash.start();
}
