 ...
 * @author jone
 */
public class AbstractSale implements Callable<Object> {

    private static final String KEY_STORE = "PKCS12";//keystore格式
    private Count c = null;   //记录数
    private final Logger logger = Logger.getLogger(AbstractSale.class);
    private AgentAbstractUserReqList req = null;   //投注上传报文
    private AppHeader head = null;  //终端
    private int doNum = 1;   //没终端执行次数

    @SuppressWarnings("unused")
    private AbstractSale() {
    }

    public AbstractSale(AgentAbstractUserReqList reqBean, AppHeader head, Count c, int doNum) {
        this.req = reqBean;
        this.head = head;
        this.c = c;
        this.doNum = doNum;
    ...
    public Object call() throws Exception {
        return execAbstract(req, head, c, doNum);
    }
     ...
     */
    private int execAbstract(final AgentAbstractUserReqList tbr, final AppHeader head, final Count c, final int doNum) throws DecoderException {
        try {
...
//                tbr.setGambler_serial_no("" + time.substring(5) + new Random().nextInt(999) + tm.substring(0,3));
            ...
            PAgentAbstractUserClient betScheme = new PAgentAbstractUserClient();
...
//                }
            ...
            requestxml = SerialNo.getxml(requestxml, ActionID.DEALER_PROXYABSTRACT);
            BetNo bn = new BetNo();
            // bn.send(requestxml);
            if (bn.Abstractsend(requestxml) != 0) {//发送数据
                c.addfailcount();
                // continue;
            } else {
                c.addCount();
