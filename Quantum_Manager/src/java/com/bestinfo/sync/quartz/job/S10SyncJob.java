...
@Component
public class S10SyncJob {

    private final Logger logger = Logger.getLogger(S10SyncJob.class);
    ...
    public synchronized void masterThread() {
        logger.warn("******************** S10 sync job start ********************");
        logger.warn("game_code:" + SyncInfo.S10);

        GameInfo gameInfo = gameInfoDao.getGameInfoByGameCode(metaJdbcTemplate, SyncInfo.S10);
        if (gameInfo == null) {
            logger.error("^^^^^ GameInfo is null,game_code:" + SyncInfo.S10 + " ^^^^^");
            return;
        }
        //判断游戏是否可用
        if (gameInfo.getUsed_mark() != WorkStatusDefine.work) {
            logger.warn("^^^^^ GameInfo is unavailable,game_code:" + SyncInfo.S10 + " ^^^^^");
            return;
        }
        ...
        logger.warn("begin creating S10 sync file,today:" + drawName);
        ...
        while (true) {
            if (kdrawList == null || kdrawList.isEmpty()) {
                logger.warn("all keno draw have started S10 sync thread");
                break;
            }
            ...
            if (sub_time > 0) {
                ...
                try {
                    logger.warn("sales is not over,S10 quartz needs to sleep:" + sub_time + "ms");
        ...
        logger.warn("******************** S10 sync job complete ********************");
    }
}
