...
import com.bestinfo.service.game.IGameDrawInfoGambService;
...
public class GameDrawInfoGambController {

    private static final Logger logger = Logger.getLogger(GameDrawInfoGambController.class);

    @Resource
    private IGameDrawInfoGambService gameDrawInfoGambService;
    ...
    public String toModify(HttpServletRequest request, Model resModel) {
        ...
        if (game_id > 0 && draw_id > 0) {
            GameDrawInfo gameDrawInfo = gameDrawInfoGambService.getDrawByGameIdAndDrawId(game_id, draw_id);
    ...
    Map<String, Object> preScheduleDraw(HttpServletRequest request) {
        ...
        int result = gameDrawInfoGambService.preScheduleDraw(game_id, draw_no);
        if (result < 0) {
...
//    }
    ...
    Map<String, Object> newDraw(HttpServletRequest request) {
        ...
        int result = gameDrawInfoGambService.newDraw(game_id);
        if (result == 0) {
            resMap.put("code", "0");
...
//        }
    ...
    Map<String, Object> modifyGameDrawInfo(HttpServletRequest request, Model resModel, GameDrawInfo drawInfo) {
        ...
        try {
            ...
            //获取数据库中的期次信息
            GameDrawInfo dbDrawInfo = gameDrawInfoGambService.getDrawByGameIdAndDrawId(drawInfo.getGame_id(), drawInfo.getDraw_id());
            if (dbDrawInfo == null) {
            ...
            int result = gameDrawInfoGambService.updateGameDrawInfo(dbDrawInfo);
            if (result == 0) {
    ...
    Map<String, Object> cashAuth(HttpServletRequest request) {
        ...
        //更新期次状态为兑奖期，并打开终端封机
        int result = gameDrawInfoGambService.cashAuth(game_id, draw_id);
        if (result == 0) {
            resMap.put("code", "0");
    ...
    Map<String, Object> cashBatchAuth(HttpServletRequest request) {
        ...
        //更新期次状态为兑奖期，并打开终端封机
        int result = gameDrawInfoGambService.cashBatchAuth(gameDrawList);
        if (result == 0) {
            resMap.put("code", "0");
            resMap.put("msg", "兑奖授权成功！");
        } else {
    ...
    public String gameDrawPage(HttpServletRequest request, Model resModel) {
        ...
        Pagination<GameDrawInfo> page = gameDrawInfoGambService.getGameDrawInfoPageList(params);
    ...
    /**
     * 排期 测试url：http://localhost:8080/Quantum_Manager/gamedraw/preDraw?game_id=1
     *
     * @param request
     * @return
     */
    @RequestMapping(value = "/preDraw")
    public @ResponseBody
    Map<String, Object> preDraw(HttpServletRequest request) {
        logger.info("pre schedule game draw");
        ...
        int game_id = ServletRequestUtils.getIntParameter(request, "game_id", 0);
        if (game_id <= 0) {
            logger.error("game_id:" + game_id);
            resMap.put("code", -1);
            resMap.put("msg", "数据异常");
            return resMap;
        }

        String drawParameter = ServletRequestUtils.getStringParameter(request, "drawParameter", "0");
        String drawName = ServletRequestUtils.getStringParameter(request, "drawName", "");
        String salesBegin = ServletRequestUtils.getStringParameter(request, "salesBegin", "");
        String salesEnd = ServletRequestUtils.getStringParameter(request, "salesEnd", "");
        String cashEnd = ServletRequestUtils.getStringParameter(request, "cashEnd", "");
        String overdueNoDetail = ServletRequestUtils.getStringParameter(request, "overdueNoDetail", "");
        logger.info("overdueNoDetail:" + overdueNoDetail);

        int result = gameDrawInfoGambService.preDraw(game_id, drawParameter, drawName, salesBegin, salesEnd, cashEnd, overdueNoDetail);
        if (result < 0) {
            resMap.put("code", result);
            resMap.put("msg", "排期异常");
        } else {
