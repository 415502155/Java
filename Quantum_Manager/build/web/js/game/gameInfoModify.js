$(function() {
    ...
    //相关游戏的选中状态
    var bindGameId = $("#bindGameId").val();
    setTimeout(function(){
         $("#bind_game_id option[value=" + bindGameId + "]").attr("selected", "true");
    },500);
//    $("#bind_game_id option[value=" + bindGameId + "]").attr("selected", "true");
    ...
    $("#modify_button").click(function() {
        ...
        $.ajax({
            cache: true,
            type: "POST",
            url: contextPath + "/gameinfo/modify?draw_period="+draw_period,
...
function checkParams() {
    ...
    if (!checkLen("game_type", 4)) {  //YangRong改，原值：2
        alert("游戏类型字符长度不能超过2");
        return false;
    }
    if (!isNull("game_name")) {
