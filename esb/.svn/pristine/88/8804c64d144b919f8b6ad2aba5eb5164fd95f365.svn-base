...
function initUI(_box){
	...
	$("table.treetable", $p).treetable({ expandable: true });	
	...
	$("a[rel=asort]",$p).click(function(event){
		$this = $(this);
		var sort = $("input[name=asort]",$this.parent()).val();
		var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
		DWZ.debug(url);
		if (!url.isFinishedTm()) {
			alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
		...
		ajaxTodo(url+"&sort="+sort);
		event.preventDefault();
	});
	$("a[rel=msort]",$p).click(function(event){
		$this = $(this);
		var sort = $("input[name=msort]",$this.parent()).val();
		var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
		DWZ.debug(url);
		if (!url.isFinishedTm()) {
			alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
