...
public class TmnInfoDaoImpl extends BaseDaoImpl implements ITmnInfoDao {
    ...
    private static final String UPDATE_TMN_INFO_BY_ID="update t_tmn_info set  public_key = ? where terminal_id = ?";
    private static final String GET_TMN_INFO_LIST_BY_DEALERID = "select a.* from t_tmn_info a where  terminal_type=3 and   dealer_id =?";
    private static final String SELECT_TMNINFO_BY_CITY_TERMINALTYPE = "select * from t_tmn_info where city_id=? and terminal_type=? order by terminal_id";
    /**
     * 新增投注机信息
     *
     * @param jdbcTemplate
    ...
    @Override
    public int updateTmnInfoById(JdbcTemplate jdbcTemplate, int terminalId, String publicKey) {
        int result = -1;
        try {
            result = jdbcTemplate.update(UPDATE_TMN_INFO_BY_ID, new Object[]{                
                publicKey,
                terminalId
            });
        } catch (DataAccessException e) {
     ...
     */
    @Override
    public List<TmnInfo> getOrdinaryTmnInfo(JdbcTemplate jdbcTemplate, String dealerId) {
        return this.queryForList(jdbcTemplate, GET_TMN_INFO_LIST_BY_DEALERID, new Object[]{dealerId}, TmnInfo.class);
    }
    /**
     * 获取某地市下某种类型的终端
     *
     ...
     */
    @Override
    public List<TmnInfo> getTmnByCityIdAndTerminalType(JdbcTemplate jdbcTemplate, int cityId, int terminalType) {
        return this.queryForList(jdbcTemplate, SELECT_TMNINFO_BY_CITY_TERMINALTYPE, new Object[]{cityId,terminalType}, TmnInfo.class);
    }
}
