	...
	public void pay(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(null!=cd&&null!=cd.getCid()){
			if(MoneyUtil.fromYUANtoFEN(cd.getMoney()).equals(txnAmt)){
				...
				if(isSuccess){
					LoggerUtil.save(cd.getOrg_id(), charge.getCam_id(), cd.getStud_id(), EnumLog.TARGET_TYPE_STU.getKey(), cd.getStud_name(), EnumLog.OPERATION_STU_PAID_SUCCESS.getKey(), EnumLog.OPERATION_STU_PAID_SUCCESS.getValue(), Constant.IDENTITY_PARENT, Integer.parseInt(parent_id), parent_name, null);
