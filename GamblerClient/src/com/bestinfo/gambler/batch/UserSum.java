 ...
 * @author jone
 */
public class UserSum implements Callable<Object> {
    ...
    private UserSummaryInfoReq req = null;   //投注上传报文
    ...
    public UserSum(UserSummaryInfoReq reqBean, AppHeader head, Count c, int doNum) {
        this.req = reqBean;
        this.head = head;
        this.c = c;
        this.doNum = doNum;
    ...
    public Object call() throws Exception {
        return execUserSum(req, head, c, doNum);
    }
     ...
     */
    private int execUserSum(final UserSummaryInfoReq tbr, final AppHeader head, final Count c, final int doNum) throws DecoderException {
        try {
            PUserSumQueryClient betScheme = new PUserSumQueryClient();
            ...
            requestxml = SerialNo.getxml(requestxml, ActionID.ACCOUNT_QUERY);
            BetNo bn = new BetNo();
            // bn.send(requestxml);
            if (bn.UserSumsend(requestxml) != 0) {//发送数据
                c.addfailcount();
                // continue;
            } else {
                c.addCount();
