...
//    private final Logger logger = Logger.getLogger(StatPrizeAnnDaoImpl.class);
    ...
    //不包含电话投注数据
    private static final String GET_ALL_SYSTEM_PRIZE_WITHOUT_GAMB = "select g.game_id,"
            + "       g.draw_id,"
            + "       g.play_id,"
            + "       g.open_id,"
            + "       g.class_id,"
            + "       g.class_name,"
            + "       sum(g.prize_num) + sum(s.prize_stakes) as prize_num"
    ...
    public List<StatPrizeAnn> getAllSystemPrize(JdbcTemplate jdbcTemplate, int gameid, int drawid) {
        return this.queryForList(jdbcTemplate, GET_ALL_SYSTEM_PRIZE_WITHOUT_GAMB, new Object[]{gameid, drawid, gameid, drawid}, StatPrizeAnn.class);
    }
}
