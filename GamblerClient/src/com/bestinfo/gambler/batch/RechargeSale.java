 ...
 *
 */
public class RechargeSale implements Callable<Object> {

    private static final String KEY_STORE = "PKCS12";//keystore格式
    private Count c = null;   //记录数
    private final Logger logger = Logger.getLogger(RechargeSale.class);
    private AgentRechargeUserReqList req = null;   //投注上传报文
    private AppHeader head = null;  //终端
    private int doNum = 1;   //没终端执行次数

    @SuppressWarnings("unused")
    private RechargeSale() {
    }

    public RechargeSale(AgentRechargeUserReqList reqBean, AppHeader head, Count c, int doNum) {
        this.req = reqBean;
        this.head = head;
        this.c = c;
        this.doNum = doNum;
    ...
    public Object call() throws Exception {
        return execRecharge(req, head, c, doNum);
    }
     ...
     */
    private int execRecharge(final AgentRechargeUserReqList tbr, final AppHeader head, final Count c, final int doNum) throws DecoderException {
        try {
            //logger.info("票数是："+doNum);
            // for (int j = 0; j < doNum; j++) {
            //logger.info("264791274207");
...
//                String tm = tbr.getGamblerName()+ "";
//                tbr.setGambler_serial_no("" + time.substring(5) + new Random().nextInt(999) + tm.substring(0,3));
            ...
            PAgentRechargeUserClient betScheme = new PAgentRechargeUserClient();
...
//                }
            logger.info("sessionKey is: " + StaticVariable.SESSION);
            ...
            requestxml = SerialNo.getxml(requestxml, ActionID.DEALER_AGENTRECHARGE);
            BetNo bn = new BetNo();
            // bn.send(requestxml);
            if (bn.Rechargesend(requestxml) != 0) {//发送数据
                c.addfailcount();
                // continue;
            } else {
                c.addCount();
