		...
		if (userList != null && userList.size() > 0) {
			for (User user : userList) {
				...
				if (name.equals(user_name) && pass.equals(user_pass)) {
					...
					String roleId = (String) userRoleMenuInfo.get("role_id");
					String roleName = (String) userRoleMenuInfo.get("role_name");
					String menuId = (String) userRoleMenuInfo.get("menu_id");
					...
					//校验通过
					String token = userId + "_" + CommonUtils.getUUID();//userId+uuid
					user.setToken(token);
