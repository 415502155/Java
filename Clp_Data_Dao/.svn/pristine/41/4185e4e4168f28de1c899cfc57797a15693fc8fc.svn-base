...
public class GameDrawInfoDaoImpl extends BaseDaoImpl implements IGameDrawInfoDao {
     ...
     * 根据游戏id和期号获取期次
     */
    private static final String GET_GAMEDRAWINFO_BY_GAME_ID_AND_DRAW_ID = "select game_id,draw_id,draw_name,draw_type, sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num, begin_keno_draw_id,end_keno_draw_id  from t_game_draw_info where game_id = ? and draw_id = ?";
     ...
     * 根据游戏id和期名获取期次
     */
    private static final String GET_DRAW_BY_GAME_DRAWNAME = "select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id from t_game_draw_info where game_id = ? and draw_name = ?";

    /**
     * 根据游戏id获取新期列表
     */
    private static final String GET_DRAW_BY_GAMEID_DRAWSTATUS = "select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end, cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id  from t_game_draw_info where game_id = ? and process_status=? order by draw_id asc";
     ...
     * 根据游戏id获取最大期
     */
    private static final String GET_MAX_DRAW_BY_GAME_ID = "select d.game_id,d.draw_id,d.draw_name,d.draw_type,d.sales_begin,d.sales_end,d.cash_begin,d.cash_end,d.process_status,d.keno_draw_num,d.begin_keno_draw_id,d.end_keno_draw_id, rownum rn from  (select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id from t_game_draw_info where game_id = ? order by draw_id desc) d  where rownum=1";

    /**
     * 新增期次
     */
    private static final String INSERT_GAMEDRAWINFO = "insert into t_game_draw_info(game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id) values(?,?,?,?,?,?,?,?,?,?,?,?)";

    /**
     * 根据游戏id和期号更新期信息
     */
    private static final String UPDATE_GAMEDRAWINFO = "update t_game_draw_info set draw_name=?,draw_type=?,sales_begin=?,sales_end=?,cash_begin=?,cash_end=?,process_status=?,keno_draw_num=?,begin_keno_draw_id=?,end_keno_draw_id=? where game_id=? and draw_id=?";
     ...
     * 根据游戏id获取期信息
     */
    private static final String SELECT_GAMEDRAWINFO_BYGAMEID = "select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id from t_game_draw_info where game_id = ?";

    /**
     * 根据 game_id draw_id 和draw_type 查询游戏期信息
     */
    private static final String SELECT_GAMEDRAWINFO_BYIDS = "select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id from t_game_draw_info where game_id = ? and draw_id=? and draw_type = ?";
...
//    private String SELECT_GAMEDRAWINFO_BY_CONDITIONS = "select * from t_game_draw_info where process_status = ?";
    ...
    private static final String SELECT_GAME_ID_DRAW_ID_BY_PROCESS_STATUS = "select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id from t_game_draw_info where process_status > ?";

    private static final String SELECT_GAME_DRAW_INFO_BY_PROCESSSTATUS = " select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id from t_game_draw_info where process_status in (?,?,?)";
     ...
     * 获取两个状态之间的期列表
     */
    private static final String GET_DRAWLIST_BETWEEN_2_STATUS = "select game_id,draw_id,draw_name,draw_type, sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num, begin_keno_draw_id,end_keno_draw_id  from t_game_draw_info where game_id = ? and process_status >= ? and process_status <= ? order by draw_id asc";

    /**
     * 同步期
     */
    private static final String SYN_GAMEDRAWINFO = "select game_id,draw_id,draw_name,draw_type,sales_begin,sales_end,cash_begin,cash_end,process_status,keno_draw_num,begin_keno_draw_id,end_keno_draw_id from t_game_draw_info where process_status >= ? and process_status <= ?";
    ...
    public int mergeGamedrawinfoByGameidDrawid(JdbcTemplate jdbcTemplate, GameDrawInfo gdi) {
        ...
        try {
            result = jdbcTemplate.update(MERGE_GAMEDRAWINFO_BY_GAMEID_DRAWID, new Object[]{
                gdi.getGame_id(), gdi.getDraw_id(), gdi.getDraw_name(), gdi.getDraw_type(), gdi.getSales_begin(),
                gdi.getSales_end(), gdi.getCash_begin(), gdi.getCash_end(), gdi.getProcess_status(),
                gdi.getKeno_draw_num(), gdi.getBegin_keno_draw_id(), gdi.getEnd_keno_draw_id(),
                gdi.getGame_id(), gdi.getDraw_id(), gdi.getDraw_name(), gdi.getDraw_type(), gdi.getSales_begin(),
                gdi.getSales_end(), gdi.getCash_begin(), gdi.getCash_end(), gdi.getProcess_status(),
                gdi.getKeno_draw_num(), gdi.getBegin_keno_draw_id(), gdi.getEnd_keno_draw_id()
            });
