package com.bestinfo.redis.client.jedis;
...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    public ShardedJedisSentinelPool(final GenericObjectPoolConfig poolConfig,
            List<String> masters, Set<String> sentinels, String password) {
        ...
        for (String masterName : masters) {
            ...
            while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
                ...
                if (null == master) {
                    try {
                        log
                                .severe("All sentinels down, cannot determine where is "
                                        + masterName
        ...
        if (masters.size() != 0 && masters.size() == shardMasters.size()) {
            ...
            for (String sentinel : sentinels) {
                final HostAndPort hap = toHostAndPort(Arrays.asList(sentinel
                        .split(":")));
                MasterListener masterListener = new MasterListener(masters, hap
                        .getHost(), hap.getPort());
                        ...
                        public void onMessage(String channel, String message) {
                            ...
                            if (switchMasterMsg.length > 3) {
                                ...
                                if (index >= 0) {
                                    ...
                                    log
                                            .fine("Ignoring message on +switch-master for master name "
                                                    + switchMasterMsg[0]
                                ...
                                log
                                        .severe("Invalid message received on Sentinel "
                                                + host
                                                + ":"
                                                + port
