 ...
 * @Title: springstarter
 * @author shy
 * @Description Ftp工具类 用于大文件上传到指定服务器
 * @data 2019年2月18日 下午4:34:24
 *
 */
public class FtpUtil {
	...
	public static boolean connectFtp(Ftp f) throws Exception {
		...
		int reply;
		if (f.getPort() == null) {
			ftp.connect(f.getIpAddr(), 21);//端口号 默认值21
		} else {
	...
	public static void upload(File f) throws Exception {
		if (f.isDirectory()) {
			ftp.makeDirectory(new String(f.getName().getBytes("GBK"), "ISO-8859-1"));
	...
	public static void main(String[] args) throws Exception {
		Ftp f = new Ftp();
		f.setIpAddr("127.0.0.1");// ip地址
		f.setUserName("username");// 用户名称
		f.setPwd("password");// 用户密码
