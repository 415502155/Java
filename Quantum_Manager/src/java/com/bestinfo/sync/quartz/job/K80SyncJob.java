...
@Component
public class K80SyncJob {

    private final Logger logger = Logger.getLogger(K80SyncJob.class);
    ...
    public synchronized void masterThread() {
        logger.warn("******************** K80 sync job start ********************");
        logger.warn("game_code:" + SyncInfo.K80);

        GameInfo gameInfo = gameInfoDao.getGameInfoByGameCode(metaJdbcTemplate, SyncInfo.K80);
        if (gameInfo == null) {
            logger.error("^^^^^ GameInfo is null,game_code:" + SyncInfo.K80 + " ^^^^^");
            return;
        }
        //判断游戏是否可用
        if (gameInfo.getUsed_mark() != WorkStatusDefine.work) {
            logger.warn("^^^^^ GameInfo is unavailable,game_code:" + SyncInfo.K80 + " ^^^^^");
            return;
        }
        ...
        logger.warn("begin creating K80 sync file,today:" + drawName);
        ...
        while (true) {
            if (kdrawList == null || kdrawList.isEmpty()) {
                logger.warn("all keno draw have started K80 sync thread");
                break;
            }
            ...
            if (sub_time > 0) {
                ...
                try {
                    logger.warn("sales is not over,K80 quartz needs to sleep:" + sub_time + "ms");
        ...
        logger.warn("******************** K80 sync job complete ********************");
    }
}
